...
This repository contains Ansible playbooks for deploying MVS 3.8j TK5 (Hercules
Mainframe Emulator) on Raspberry Pi devices. The deployment creates a systemd
service that runs the emulator in unattended mode.

## Architecture

The playbook consists of two main plays:

1. **Setup pi**: System-level updates for the Raspberry Pi (tagged with
   "updates")
...
- **inventory.ini**: Target hosts (currently `tk5_pi` group)
...
Variables are templated throughout, so changes to user or install directory
...
- Service is configured for automatic restart on failure, which is important for
  reliability
